...
(function(define)
  ...
  if whitelisted == true then
    ...
    Page1:NewButton("制作组", function()
      print("徽山工作室")
    end)

    Page1:NewButton("制作者", function()
      print("三岁高材生")
    end)

    Page1:NewButton("加入我们的群", function()
      print("8795512080")
    end)
    ...
    function cooperAxeDupee()
    local cooper = game.Players.LocalPlayer
    local currentSlot = cooper.CurrentSaveSlot.Value
    
    if currentSlot == -1 then
        return HONG:NOTIFY("错误", "请先加载存档", 4)
    end
    
    -- 检查是否可以重新加载
    local function canReload()
        return game:GetService("ReplicatedStorage").LoadSaveRequests.ClientMayLoad:InvokeServer(cooper)
    end
    
    if not canReload() then
        HONG:NOTIFY("复制斧头", "冷却中，请等待...", 4)
    ...
    Page7:NewButton("复制斧头(NEW)", function() 
      cooperAxeDupee()
    end)
